...
def main():
    ...
    while not game_over:
        ...
        if len(coordinate) != 2 or not (coordinate[0].isalpha() and coordinate[1].isdigit()):
            print("Invalid input.") # Validate user input
            continue

        col = ord(coordinate[0].upper()) - ord('A') # Convert input to grid column index
        row = int(coordinate[1]) - 1

        if grid[row][col][1] == "!": # If the selected cell has a mine
            game_over = True
            ...
            mines_left = sum(row.count('!') for row in grid)  # Count remaining mines
            if revealed_count == 49 - len(mines) and mines_left == 8: # All non-mine cells revealed and no remaining mines
                game_over = True
                print("Congratulations! You've won the game.")
            elif grid[row][col][1] == "!":  # Explicitly check for hitting a mine
                game_over = True
